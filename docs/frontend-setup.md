...
React and TypeScript provide a responsive community home, an about route, and a not-found route. The home deliberately shows an empty community: accounts, boards, and discussions arrive in later stages.
...
The HTTP client is read-only in this stage. Session authentication and CSRF headers for mutations will be introduced in Stage 4. It sends same-origin credentials, bounds requests to eight seconds, preserves caller cancellation, and translates errors into a consistent ApiError. Raw server error bodies are never displayed.
...
The jobs use read-only repository permissions. Browser smoke checks are currently local because they require a running backend. Remote CI has not been executed for these uncommitted Stage 3 changes; a successful local run is not claimed as a GitHub Actions result.
...
No Stage 4 authentication or discussion features were implemented.
