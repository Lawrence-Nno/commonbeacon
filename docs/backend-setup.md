...
Flyway applies V1-V11. Hibernate validates the mapped schema and never changes it. The backend provides session authentication, community discussions, moderation, knowledge articles, and search; see [REST APIs](api.md). V10 adds internal [transfer job and storage infrastructure](data-transfer-storage.md); V11 adds [current authorization and requester job/download APIs](data-transfer-access.md). Export/import creation remains unavailable.
