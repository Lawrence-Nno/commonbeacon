...
export const listJobs = (cursor: string | null, signal?: AbortSignal, personal = false, size: 1 | 20 = 20) => getJson(`${personal ? personalRoot : root}/jobs?size=${size}${personal ? "" : "&kind=COMPANY_EXPORT"}${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`, value => readPage(value, personal), signal);
