...
test("administrator publishes, reconciles live edits and archives knowledge while other roles only read", async ({ browser }, testInfo) => {
  ...
  try {
    ...
    await edit.getByLabel("Article body").fill("Draft revised <script>plain text only</script>");
    ...
    const second = await admin.newPage(); await second.goto(editorUrl); await second.getByLabel("Article body").fill("My second tab preserved draft.");
    await edit.getByLabel("Article body").fill("First tab live update."); await edit.getByRole("button", { name: "Save article" }).click();
    ...
    await expect(second.getByLabel("Article body")).toHaveValue("My second tab preserved draft.");
    await second.getByRole("button", { name: "Load latest article" }).click();
    await expect(second.getByText("First tab live update.", { exact: true })).toBeVisible();
    ...
    await read.reload(); await expect(read.getByText("My second tab preserved draft.")).toBeVisible();
    ...
    await expect(second.getByText("My second tab preserved draft.")).toHaveCount(0);
