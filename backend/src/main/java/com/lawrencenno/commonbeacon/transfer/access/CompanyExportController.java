...
@RestController
@RequestMapping("/api/v1/admin/data/exports")
public class CompanyExportController {
    public record Request(Boolean includeContacts, Boolean includeModerationHistory,
            @NotNull @AssertTrue Boolean acknowledgedPrivateContent,
            @NotBlank @Pattern(regexp="[A-Za-z0-9_-]{43}") String recentAuthGrant) {
        public Request {
            includeContacts=Boolean.TRUE.equals(includeContacts);
            includeModerationHistory=Boolean.TRUE.equals(includeModerationHistory);
        }
        @Override public String toString() { return "CompanyExportRequest[redacted]"; }
    }
    ...
    public CompanyExportController(TransferJobs jobs, TransferAccess access, RecentAuthentication recent,
                                   ObjectProvider<ArtifactStore> stores) {
    ...
    public ResponseEntity<TransferController.Summary> create(Authentication authentication, HttpServletRequest request,
        ...
        var job=TransferController.run(()->jobs.createCompanyExport(actor.id(),key,body.includeContacts(),body.includeModerationHistory(),
            ()->recent.consume(access.current(authentication,true),request.getSession(),RecentAuthentication.Scope.COMPANY_EXPORT,body.recentAuthGrant())));
